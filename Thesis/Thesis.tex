...
Traditionally this class of problems including a Debye relaxation mechanism, is addressed with the so called ADE-FDTD method, where finite differences (FD) are applied both at time and domain (TD) dicretization. The auxiliary differential equations (ADE) in the method name are the numerical discrete counterparts of Debye model, and are deduced from \eqref{eq:Polprocess}, so, at a vectorial lavel \cite{taflove}. In our starting model \eqref{eq:finalmodel}, however we consider \(\pi_k = \nabla \cdot \mathbf{P}_k\) as variables, instead of vectors \(\mathbf{P}_k\).
...
We consider a uniform dielectric material with parameters taken from \cite{simulation-transient-HVDC}, obtained through measurements on a paper insulated cable. A uniform grid displayed in figure \ref{fig:unif-grid} is adopted. The three slow polarization processes have their parameters fitted with experimental data. We start with a null initial solution, and turn on the voltage at the upper interface with the electrode. In particular we impose \(V(t)=V_0(1-e^{-t/\tau})\chi_{(0,T_{discharge}]}(t)\), where \(V_0 = 1.5\cdot 10^4\), \(\tau=2 s\) is a time constant and \(\chi\) is an indicator function. We let the system evolve for \(T_{discharge}=15000\,s\), then we short-circuit the 2 contacts and study the discharge process. No charge is expected to accumulate in the interior of the domain, but only at the interfaces with the contacts.\\
In this very simple problem it is also possible to compute analytically the total current through the Occhini circuital model \cite{4073280},\cite{simulation-transient-HVDC}. In figure \ref{fig:test1} we report some results of this test.
...
\begin{figure}
	\hspace*{-0.01\textwidth}
	\subfloat[Plot on the \(z\) axis of free charge density \(\rho\), and of \(\pi_{slow} = \nabla\cdot\mathbf{P_{slow}}\), where \(\mathbf{P_{slow}}\) is the slow polarization component of \(\mathbf{P}\)]{
		\includegraphics[width=0.5\textwidth]{pictures/homog1-pol-charges-on-z.png}
	}
	\hspace*{0.01\textwidth}
	\subfloat[Plot on the \(z\) axis of free charge density \(\rho\), of \(\pi_1\) and \(\pi_2\), where the last two are related to a separate part of the domain.]{
		\includegraphics[width=0.5\textwidth]{pictures/homog2-pol-charges-on-z.png}
	}
	\caption{Test 2: Homogeneous material. \(\varepsilon_\infty = 3.5,\ \sigma = 3.21\cdot 10^{-14}S\). Parameters for the only process are \(\chi_p = 1.970,\ \tau_p = 3.57\cdot 10^{2}s\).}
	\label{fig:test2}
