...
\begin{tikzpicture}
	...
	\begin{scope}[x={(image.south east)},y={(image.north west)}]
		...
		\draw[thick,->] (0.4,-0.5) -- (0.9,-0.5);
		\draw[thick,->] (0.4,-0.5) -- (0.4,-0.2);
		\node (longdir) at (1.1,-0.5) {Cable longitudinal direction};
		\node (longdir) at (0.565,-0.25) {Cable radial direction};
	\end{scope}
\end{tikzpicture}
...
In the proposed model we can differentiate currents into:
\begin{itemize}
	\item \textit{Conduction current}: or drift current, generated by free charges moved by the electric field. It can be determined through the gradient of the electric potential \(\varphi\) and the conductivity \(\sigma\).
	\item \textit{Displacement current}: generated my the polarization of the dielectric resulting in the gathering of polarization charges at the interface with conductors. Since we consider also non-instantaneous response of the material, we have to distinguish between polarization charges resulting form the instantaneous polarization, and those resulting from slow polarization processes
	\begin{itemize}
		\item \textit{Instantaneous polarization}: 
	\end{itemize}
\end{itemize}
%-----------------------------------------------------------------------------
% EQUATIONS
